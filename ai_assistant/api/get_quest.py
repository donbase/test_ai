...
async def get_quest_id(
    ...
    prompt = f"""Придумай квест для ребёнка {age} лет, направленный на развитие полезных привычек и навыков.
    ...
    Гнездо Спокойствия
    Собери игрушки и подготовь комнату ко сну. Верни результат в формате JSON:
    {{
      "title": "Название",
      "task": "Описание задания"
    }}
    """
    response = eval(_llm.invoke(prompt).content)
    return response
